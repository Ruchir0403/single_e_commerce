...
export async function deleteProduct(id: string) {
  const res = await fetch(`https://backend-sin-ecom.onrender.com/api/products/${id}`, {
    method: 'DELETE',
    credentials: 'include',
  });
  if (!res.ok) throw new Error('Failed to delete product');
...
export async function fetchCart() {
  const res = await fetch('https://backend-sin-ecom.onrender.com/api/cart', {
    credentials: 'include',
  });
  if (!res.ok) throw new Error('Failed to fetch cart');
  return res.json();
...
export async function removeFromCart(cartItemId: string) {
  const res = await fetch(`https://backend-sin-ecom.onrender.com/api/cart/${cartItemId}`, {
    method: 'DELETE',
    credentials: 'include',
  });
  if (!res.ok) throw new Error('Failed to remove from cart');
